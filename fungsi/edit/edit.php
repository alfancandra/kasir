...
if(!empty($_SESSION['admin']) || !empty($_SESSION['id_kasir'])){
	...
	if(!empty($_GET['jual_kasir'])){
		...
		if($hasil['stok'] > $jumlah)
		{
			...
			$row1 = $config -> prepare($sql1);
			$row1 -> execute($data1);
			echo '<script>window.location="../../kasir/index.php?page=jual#keranjang"</script>';
		}else{
			echo '<script>alert("Keranjang Melebihi Stok Barang Anda !");
					window.location="../../kasir/index.php?page=jual#keranjang"</script>';
		}
...
<?php	
	...
	if(!empty($_GET['cari_barang_kasir'])){
		$cari = trim(strip_tags($_POST['keyword']));
		if($cari == '')
		{
